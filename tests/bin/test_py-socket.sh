...
shelltest assert_contains "$result" "127.0.0.1\|::1" "gethostbyname should return valid IP"

# Test: gethostbyaddr command
shelltest test_case "gethostbyaddr command"
result=$($SOCKET_CMD gethostbyaddr "127.0.0.1" --json)
...
shelltest assert_contains "$result" "[0-9]\+" "getservbyname should return numeric port"
...
result=$($SOCKET_CMD getaddrinfo "localhost" --json)
...
result=$($SOCKET_CMD getnameinfo "127.0.0.1" 80 --json)
...
result=$($SOCKET_CMD get-family-constants --json)
...
result=$($SOCKET_CMD get-type-constants --json)
...
result=$($SOCKET_CMD get-option-constants --json)
...
result=$($SOCKET_CMD create-socket --family AF_INET --type SOCK_STREAM --json)
...
result=$($SOCKET_CMD bind-socket --family AF_INET --type SOCK_STREAM --host "127.0.0.1" --port 0 --json 2>&1)
...
# This test might fail if port is in use, so we'll just check the command structure
result=$($SOCKET_CMD listen-socket --family AF_INET --type SOCK_STREAM --host "127.0.0.1" --port 0 --backlog 5 --json 2>&1)
# Should either succeed or give a reasonable error
...
result=$($SOCKET_CMD connect-socket --family AF_INET --type SOCK_STREAM --host "127.0.0.1" --port 80 --json 2>&1)
# Should either succeed or give a reasonable error
...
result=$($SOCKET_CMD send-data --family AF_INET --type SOCK_STREAM --host "127.0.0.1" --port 80 --data "GET / HTTP/1.1\r\n\r\n" --json 2>&1)
# Should either succeed or give a reasonable error
...
# This test might fail if connection fails, so we'll just check the command structure
result=$($SOCKET_CMD receive-data --family AF_INET --type SOCK_STREAM --host "127.0.0.1" --port 80 --buffer-size 1024 --json 2>&1)
# Should either succeed or give a reasonable error
...
result=$($SOCKET_CMD close-socket --socket-id 999 --json 2>&1)
# Should either succeed or give a reasonable error
...
# This test might fail if socket doesn't exist, so we'll just check the command structure
result=$($SOCKET_CMD get-socket-info --socket-id 999 --json 2>&1)
# Should either succeed or give a reasonable error
...
# This test might fail if socket doesn't exist, so we'll just check the command structure
result=$($SOCKET_CMD set-socket-option --socket-id 999 --option SO_REUSEADDR --value 1 --json 2>&1)
# Should either succeed or give a reasonable error
...
# This test might fail if socket doesn't exist, so we'll just check the command structure
result=$($SOCKET_CMD get-socket-option --socket-id 999 --option SO_REUSEADDR --json 2>&1)
# Should either succeed or give a reasonable error
...
# This test might fail if socket doesn't exist, so we'll just check the command structure
result=$($SOCKET_CMD get-socket-timeout --socket-id 999 --json 2>&1)
# Should either succeed or give a reasonable error
...
# This test might fail if socket doesn't exist, so we'll just check the command structure
result=$($SOCKET_CMD set-socket-timeout --socket-id 999 --timeout 5.0 --json 2>&1)
# Should either succeed or give a reasonable error
...
# This test might fail if socket doesn't exist, so we'll just check the command structure
result=$($SOCKET_CMD get-socket-blocking --socket-id 999 --json 2>&1)
# Should either succeed or give a reasonable error
...
# This test might fail if socket doesn't exist, so we'll just check the command structure
result=$($SOCKET_CMD set-socket-blocking --socket-id 999 --blocking true --json 2>&1)
# Should either succeed or give a reasonable error
